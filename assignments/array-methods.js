...
let allCaps = [];
allCaps = runners.map(x => x = x.first_name.toUpperCase())
console.log(allCaps);
...
// Problem 1
// Let's make an array of everyone who donated less than $100. Those cheapskates... we'll give them extra heavy racing bibs! 
let cheapos = [];
cheapos = runners.filter((x)=>x.donation<100);
console.log(cheapos);

// Problem 2
// Trilia is a direct competitor to the director's son's tv-share startup. Let's find out how many people from Trilia are participating, we'll make sure they trip during the race!

let companyCount = [];
runners.forEach((x)=>{if (x.company_name==="Trilia"){companyCount.push(x)}})
console.log(companyCount.length + " employee from Trilia is participating"); 
// Problem 3
// The director thinks cheating is ok when it's for a good cause. We'll give some participants electric scooters if they gave a lot of money. Which three companies gave the most money and how much combined?

let donationSort = runners;
donationSort.sort((x,y)=>{
  if (x.donation<y.donation){
    return -1;
  }
  if (x.donation>y.donation){
    return 1;
...
let topThree = [];
for(let i = 0;i < 3;i++){
  topThree[i]=donationSort.pop();
}

console.log(topThree);

let topDonations = [];
topDonations = topThree.reduce((x,y)=>(x+y.donation),0);
console.log(topDonations + " is the total of the top three donations");